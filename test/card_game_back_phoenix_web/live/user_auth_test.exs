defmodule CardGameBackPhoenixWeb.Live.UserAuthTest do
  ...
  alias CardGameBackPhoenixWeb.Live.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CardGameBackPhoenixWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to settings when user is already logged in", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/live/users/settings"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CardGameBackPhoenixWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: CardGameBackPhoenixWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CardGameBackPhoenixWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
